...
// Versão autocontida para evitar falhas silenciosas na cadeia de imports.
...
function validateBank(raw) {
  ...
  for (const item of source) {
    ...
    if (!Number.isInteger(id)) { warnings.push(`${prefix}: id inválido.`); continue; }
    if (ids.has(id)) { warnings.push(`${prefix}: id duplicado.`); continue; }
    ids.add(id);

    if (!ANSWER_PATTERN.test(answer)) {
      warnings.push(`${prefix}: resposta inválida (${JSON.stringify(answer)}).`);
      continue;
    }
    if (answer.length < 2) { warnings.push(`${prefix}: resposta curta demais.`); continue; }
    if (!clue) { warnings.push(`${prefix}: dica vazia.`); continue; }
    if (!theme) { warnings.push(`${prefix}: tema vazio.`); continue; }
    if (![1, 2, 3].includes(difficulty)) { warnings.push(`${prefix}: dificuldade inválida.`); continue; }
    ...
    words.push({
      id,
      resposta: answer,
      exibicao: typeof item?.exibicao === "string" && item.exibicao.trim() ? item.exibicao.trim() : answer,
      dica: clue,
      tema: theme,
...
async function loadWordBank(url) {
  ...
  if (!response.ok) {
    throw new Error(`Falha ao carregar ${absoluteUrl.pathname}: HTTP ${response.status}`);
  }
...
function getAvailableThemes(bank) {
  const themes = [...new Set(bank.filter((word) => word.tema !== "Coringa").map((word) => word.tema))];
...
function filterByTheme(bank, theme) {
  if (theme === ALL_THEMES) return bank;
  return bank.filter((word) => word.tema === theme || word.tema === "Coringa");
}
